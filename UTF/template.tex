...
% are deemed to useful as additional source of libraries co-usage patterns.
% In addition, we can infer additional library co-usage patterns which can be used in third-party library recommendation system as additional resource.
...
% \begin{table}[h]
% \centering
% 	\caption{The tags used to identify npm related posts}
% 	\resizebox{\columnwidth}{!}{%
%     \begin{tabular}{lrrrr}
%         \hline
%         Filtered tag & \# Initial posts & \# all SO posts & TRT (\%) & TST (\%) \\ 
%         \hline
%         npm & 89 & 29100 & 0.31 & 100.00 \\
%         package.json & 7 & 1612 & 0.43 & 7.87 \\
%         npm-install & 4 & 2941 & 0.14 & 4.49 \\
%         npm-scripts & 2 & 526 & 0.38 & 2.25 \\
%         npmignore & 1 & 19 & 5.26 & 1.12 \\
%         npm-shrinkwrap & 1 & 55 & 1.82 & 1.12 \\ 
%         \hline
%     \end{tabular}%
%     }
% 	\label{tab:tags}
...
% \textit{\textbf{Step 1: Collect initial posts}}. SO posts are typically tagged by relevant tags to improve visibility of the posts. 
% Following the similar approach that was used in previous study~\cite{rosen2016mobile}, we collect the initial npm dataset by filtering posts that contain both \texttt{npm} and \texttt{package-managers} as tag words. 
% The output of this step is 89 npm related posts.

% \textit{\textbf{Step 2: Discover relevant tags}}. In this step, we extract all the tags from 89 posts to discover relevant tags. 
% One major risk of discovering relevant tag is the possibility of introducing noise in the main dataset.
% For example, \textit{Global webpack install vs. project specific webpack install} is a relavant post that contains tag words \texttt{package} and \texttt{npm-install} along with \texttt{npm} and \texttt{package-managers}. Here, the tag \texttt{package} will introduce noise if we add it in the relevant tag list.  Therefore, to mitigate this problem, 
% we group and aggregate the target tags through semi-automatic process. In detail, the process includes string search using \texttt{npm} tag and manual verification of newly explored tags by reading their description on the SO. In addition, we validate the tags by applying tag relevance threshold (TRT) and  tag significance threshold (TST) as metrics to validate tags:

% \begin{eqnarray}
%     TRT_{tag} & = & \frac{\# tag\_posts}{Sum ( \# tag\_posts)} \\
%     TST_{tag} & = & \frac{\#\ tag\_posts}{\# popular\_tag\_posts }
% \end{eqnarray}

% \noindent where \textit{\#tag\_posts} is the total number of npm posts for the tag, \textit{Sum(\#tag\_posts)} is the total number of posts for the tag, and \textit{\#popular\_tag\_posts} means number of npm posts for the most popular tag.
% For instance, \texttt{package.json} is a tag word that occur only 7 times as a co-occurring tag in npm post while the total number of question posts on SO is 1612. Therefore, we also included such kind of tags in the final tag set. Thus, the output of step 2 is manually validated 6 tags (see Table~\ref{tab:tags}) as being representative of npm posts.


% \textit{\textbf{Step 3: Collect tagged posts}}. 
% After obtaining the npm-related tag set, we use those tags to identify and extract posts to create final npm post dataset. The output of step 3 is 30136 question and 37671 answer posts from SO being used as final dataset.

% \textit{\textbf{Step 4: Extract the co-usage package}}. Once we obtain the final npm dataset, we apply the top 100 npm libraries to search the library co-usage. We extract the co-usage library if a post body contain at-least two libraries from top 100 npm library list. To mitigate false positive we only extract library from post body for $<code>...</code>$ and $<pre><code>...</code></pre>$. Thus we obtain 8012 co-usage library transaction rows from SO which will be used in the subsequent sections.
...
\begin{figure}[t]
    ...
    \begin{axis}[
     ...
     \legend{Top 15 co-usage libraries}
...
\begin{itemize}
    % \item Extract co-usage libraries from SO posts and real projects dataset.
...
% \subsubsection{SO co-usage library collection}
% Figure~\ref{fig:method} shows overview of the methodology of collecting data which is described as follows:  Initially, we downloaded the latest SO data dump (July 2008 to December 2019) that is publicly available on the SOTorrent~\cite{Baltes2018SoTorrent}.
% The SO data contains all the {Q\&A} with metadata of the posts (e.g., creation date, view count, favourite count, score, etc.). 
% The initial collected dataset contains 46,947,633 posts, where 39.83\% (18,699,426) are question posts and 60.17\% (28,248,207) are answer posts spanning from July 2008 to December 2019. We collect specific package related post using the following steps:
% \begin{itemize}
%     \item Step 1: Collect initial posts
%     \item Step 2: Discover relevant tags
%     \item Step 3: Collect tagged posts
%     \item Step 4: Extract the co-usage package
% \end{itemize}



% The detail of each step is described in subsection~\ref{subsec:DatasetD1}.
...
% \begin{figure}[ht]
% 	\centering
% 	\includegraphics[width=0.34\textwidth]{matrix.pdf}
% 	\caption{An example of SO thread-libraries relationship matrix } 
% 	\label{fig:method1}
% \end{figure}

% \begin{figure}[ht]
% 	\centering
% 	\includegraphics[width=0.15\textwidth, height=1cm]{graph_new.pdf}
% 	\caption{Graph representation of SO threads and libraries} 
% 	\label{fig:graph}
% \end{figure}
...
\begin{itemize}
        ...
        % We suppose that SOLibRec with more data as query enhance the overall performance. The hypothesis is increase of dataset size using SO npm library co-usage dataset will help to improve its performance.
    \end{itemize}
...
% Since SOLibRec leverage the SO and libraries.io projects co-usage libraries dataset  to recommend libraries, we perform an experiment based on realistic scenario.
...
%     \begin{itemize}
%         \item{\RqOne} We evaluate the SOLibRec by considering SO co-usage third-party library and real npm projects.  
